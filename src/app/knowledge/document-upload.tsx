...
export function DocumentUpload() {
  ...
  const uploadFile = async (file: File, fileId: string) => {
    try {
      ...
      toast.success(
        `Successfully processed ${result.title} (${result.chunksCreated} chunks created)`
      );
    } catch (error) {
      const errorMessage =
  ...
  const processFile = async (file: File) => {
    ...
    if (!supportedTypes.includes(file.type)) {
      notificationUtils.general.error(
        "Unsupported file type",
        `${file.type}. Please upload PDF, DOCX, or TXT files.`
      );
      return;
    }
    ...
    if (file.size > maxSize) {
      notificationUtils.general.error(
        "File too large",
        `${file.name}. Maximum size is 10MB.`
      );
      return;
    }
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Upload documents to use with RAG (PDF, DOCX, TXT)
        </CardDescription>
      </CardHeader>
      <CardContent>
        <div
